...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* ━━━ メインビジュアルバナー ━━━ */}
      <section className="relative overflow-hidden bg-gradient-to-br from-[#e8f4fc] via-[#f0f7fc] to-[#eaf6f0]">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {/* 装飾アイコン群 */}
          <div className="absolute top-8 right-[10%] text-4xl opacity-30 hidden md:block">📊</div>
          <div className="absolute top-16 right-[25%] text-3xl opacity-25 hidden md:block">💰</div>
          <div className="absolute bottom-12 right-[15%] text-3xl opacity-20 hidden md:block">📋</div>
          <div className="absolute top-12 left-[8%] text-2xl opacity-20 hidden md:block">⚙️</div>
          {/* 右側のビジネスイラスト風グラデーション */}
          <div className="absolute top-0 right-0 w-[45%] h-full bg-gradient-to-l from-blue-100/40 to-transparent hidden md:block" />
        </div>

        <div className="relative max-w-5xl mx-auto px-4 py-10 md:py-16 text-center">
          {/* サブコピー */}
          <div className="inline-block bg-white/80 backdrop-blur-sm border border-blue-200 rounded-full px-5 py-2 mb-6 shadow-sm">
            <p className="text-sm md:text-base font-bold text-gray-700">
              利用者に評価された本当に良いファクタリング会社に出会える！
            </p>
          </div>

          {/* メインコピー */}
          <h1 className="text-2xl md:text-[2.8rem] font-black leading-[1.3] text-gray-900 tracking-tight mb-2">
            <span className="relative inline-block">
              <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-orange-400">国内最大級</span>
              <span className="absolute bottom-0 left-0 w-full h-2 md:h-3 bg-orange-200/60 rounded-full -z-0" />
            </span>
            のファクタリング会社の
          </h1>
          <p className="text-3xl md:text-[3.5rem] font-black leading-[1.2] text-gray-900 tracking-tight mb-8">
            口コミ・比較サイト
          </p>
          ...
          {/* 信頼バッジ */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-4 text-xs text-gray-400 justify-center">
            ...
            <span className="inline-flex items-center gap-1"><svg className="w-3.5 h-3.5 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>{companies.length}社掲載</span>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-blue-50 via-white to-emerald-50/40">
        <div className="relative max-w-5xl mx-auto px-4 pt-8 md:pt-12 pb-6 md:pb-8">
          {/* 統計バー */}
